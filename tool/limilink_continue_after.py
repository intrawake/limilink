...
def main():
    ...
    if c_pid == 0:
        ...
        notify(f"[{status}] Command completed: {command_str}")
        sys.exit(ret)
    else:
        # Parent: The Waiter
        start_time = time.time()
        while time.time() - start_time < args.delay:
            # Check if child (monitor) has exited
            pid, status = os.waitpid(c_pid, os.WNOHANG)
            if pid != 0:
                # Child (and thus the command) finished within the delay
